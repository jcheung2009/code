...
for i = 26:length(ff)
    cd(ff(i).name);
    cmd1 = ['fv_syllA_',ff(i).name,'=','jc_findwnote5(''batch.keep'',''a'','''',''b'',[0.02 0.03],fvalbnd_syllA,512,1,''w'',0,0)'];
    cmd2 = ['fv_syllB_',ff(i).name,'=','jc_findwnote5(''batch.keep'',''b'','''',''c'',[0.02 0.03],fvalbnd_syllB,512,1,''w'',0,0)'];
    %cmd3 = ['fv_syllC_',ff(i).name,'=','jc_findwnote5(''batch.keep'',''c'','''','''',[0.02 0.04],fvalbnd_syllC,512,1,''obs0'',0,0)'];
    cmd4 = ['fv_syllD_',ff(i).name,'=','jc_findwnote5(''batch.keep'',''d'',''c'','''',[0.02 0.04],fvalbnd_syllD,512,1,''w'',0,0)'];
