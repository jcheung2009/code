...
%params and input
load('dur_correlation_analysis_burst.mat');
...
minsampsize = min([corrmat_gap(idgap,7);corrmat_dur(iddur,7)]);
maxsampsize = max([corrmat_gap(idgap,7);corrmat_dur(iddur,7)]);
figure;hold on;
[n b] = hist(corrmat_gap(idgap,7),[minsampsize:10:maxsampsize]);
stairs(b,n/sum(n),'k','linewidth',2);
[n b] = hist(corrmat_dur(iddur,7),[minsampsize:10:maxsampsize]);
stairs(b,n/sum(n),'r','linewidth',2);
legend('gap single units','syllable single units');
[h p] = kstest2(corrmat_gap(idgap,7),corrmat_dur(iddur,7));
text(0,1,{['p(ks)=',num2str(p)]},'units','normalized','verticalalignment','top');
xlabel('sample size');ylabel('probability');
