function feature_vecs = jc_measure_syll_features(batch,classes)
...
for i = 1:length(classes)
    ...
    for ii = 1:length(ff)
        ...
        for ind = 1:length(p)
            ...
            dur = toff-ton;
            pxx = pxx/sum(pxx);
            spec_ent = -sum(pxx.*log2(pxx))/log2(length(pxx));
            freq1 = mean(pxx(f<1000));
            ...
            sm = sm/sum(sm);
            tempo_ent = -sum(sm.*log2(sm))/log2(length(sm));
