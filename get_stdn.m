function [stdsp,t,f,avsm]=get_stdn(bt,NT,PRETIME,POSTTIME,PRENT,POSTNT,CS,PLTIT);
...
for ii=1:length(files)
	...
	if (length(pp)>0)
		...
		for jj=1:length(pp)
			...
			if(spcnt==0)
				avsp=abs(sp);
                stdsp = zeros(size(sp,1),size(sp,2),length(files));
                stdsp(:,:,1) = avsp;
				avsm=sm;
				spcnt=1;
			else
...
stdsp = stdsp(:,:,1:spcnt);
%stdsp = std(stdsp,0,3);
stdsp = zscore(stdsp,0,3);
stdsp = mean(abs(stdsp),3);
