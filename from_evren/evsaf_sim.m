function [vol_list,ent_list,ratlist] = evsaf_sim(cbin_in,reEval)
...
% mean_Ratio = mean(vol_list(songind)) / mean(vol_list(nosongind))
% Entropy_Ratio = mean(entlist(songind)) / mean(entlist(nosongind))

song_amp = vol_list(songind);
song_ent = entlist(songind);
