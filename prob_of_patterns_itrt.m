function [ptnprobstrct,uniqueptnsvect] = prob_of_patterns_itrt(ptnstrct,cndnt)
...
ptnprobstrct(index1+1).ptn = [];
ptnprobstrct(index1+1).nmbunqptns = index1;
...
%calculate appropriat conditional probabilities and counts per song
	...
	for k = 1:index1
        ...
        if isempty(x)
            cndnotevect = [cndnotevect cndnte];
            cndntecnt(length(cndnotevect)) = ptnprobstrct(k).cnt;
        else
	...
	for p = 1:index1
        ...
        cndnte = ptn(pos);
        x = findstr(cndnotevect,char(cndnte));
        ptnprobstrct(p).cndnt = char(cndnte);
