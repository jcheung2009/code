...
% Comodugram - looks for comodulations in power spectra of a multiple time series (with correlation coefficient)
% Comodugram2 - does the same thing in a differnent way (with E(XY) / (E(X)*E(Y))
% Comoherogram - looks for comodulations in power spectra and coherences of a multiple time series
% ComplexImage - plots a pseudocolor plot of a complex matrix where brightness is amplitude and hue is phase
...
% MultiCohere - takes a multivariate time series and does a matrix of coherence plots
% MultiReg - does lots of linear regressions at the same time, so you can regress an entire waveform onto a single variable
% NormVector - Takes a matrix that consists of column vectors and normalized each one. 
...
% RandomSample - produces a vector of m integers in the range 1:n without replacement
...
% cov2 - covariance matrix for two multivariate data sets
...
% mtchgnorm - Multi-taper time-frequency coherence-gram, with normalizing transformation
