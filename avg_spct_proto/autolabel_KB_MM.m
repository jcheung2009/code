function autolabel_KB_MM(batchsongs,filetype,name,do_fltflt)
...
templates = zeros(size(templateSylls,1),17);

for i = 1:(size(templateSylls,1)-1)
    templates(i,:) = feature_vect(templateSylls{i},fs);
    %templates = [templates; feature_vect(templateSylls{i},fs)];
    %disp(i);
    if mod(i,10)==0
