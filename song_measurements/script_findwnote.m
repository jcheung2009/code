...
for i = 1:length(ff)
    cd(ff(i).name);
    cmd1 = ['fv_syllA_',ff(i).name,'=','jc_findwnote5(''batch.keep'',''a'','''','''',[0.02 0.04],fvalbnd_syllA,512,1,''obs0'',0,0)'];
    cmd2 = ['fv_syllC_',ff(i).name,'=','jc_findwnote5(''batch.keep'',''c'','''','''',[0.015 0.035],fvalbnd_syllC,512,1,''obs0'',0,0)'];
    cmd3 = ['fv_syllR_',ff(i).name,'=','jc_findwnote5(''batch.keep'',''r'','''','''',[0.04 0.06],fvalbnd_syllR,512,1,''obs0'',0,0)'];
    ...
    varname2 = ['''fv_syllC_',ff(i).name,''''];
    varname3 = ['''fv_syllR_',ff(i).name,''''];
    ...
    clearvars -except fvalbnd_syllA fvalbnd_syllC fvalbnd_syllR fvalbnd_syllW 
    cd ../../
end
toc
