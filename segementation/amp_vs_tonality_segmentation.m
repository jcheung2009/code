function motifsegment = amp_vs_tonality_segmentation(batch,params,CHANSPEC);
...
%         sm2 = sm2./max(sm2);
        ...
        tonality=tonality-mean(tonality);
        [ons offs] = SegmentNotes(tonality,fs/downsamp,minint,mindur,0);
         if length(ons) ~= length(motif)
             sylldurations2 = [];
             gapdurations2 = [];
             motifduration2 = NaN;
     ...
     motifsegment(motif_cnt).tonality=tonality;
    end
