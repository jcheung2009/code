function patternstruct = get_pattern_ptn(lbls,ptn)


%  get_pattern_ptn:    searches through a string of notes for a variable pattern
%  
%  inputs:
%     <lbls>: a string of note labels that will be searched through
%     <ptn>: the pattern to match
%
%     SYNTAX FOR PATTERN:
...
% [labels, ons, offs, fname] = get_notes_ptn_ptn(batch);
...
sz = sz(1);
for g = 1:sz
    labels = ['/' char(lbls(g,:)) labels];
    ons = [0 1:length(char(lbls(g,:))) ons];
    offs = [0 2.*(1:length(char(lbls(g,:)))) offs];
    fname{g} = 'null';
end
...
%save the strct??
% yn_save = input('Do you want ot save the pattern struct? [dflt = 0] ');
% if ~isempty(yn_save) && yn_save
