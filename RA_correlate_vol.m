function [spk_vol_corr case_name dattable] = RA_correlate_vol(batchfile,seqlen,...
    minsampsize,activitythresh,motorwin,plotcasecondition,shuff,mode,ifr,gap_or_syll)
...
dattable = table([],[],[],[],[],[],[],'VariableNames',{'spikes',...
        ...
        if strcmp(mode,'burst')
            ...
            for ixx = 1:length(pkid)
                ...
                if ~isempty(strfind(shuff,'y'))
                    ...
                    [r1 p1] = shuffle(npks_burst,vol1,shufftrials);
                    [r2 p2] = shuffle(npks_burst,vol2,shufftrials);
                    spk_vol_corr = [spk_vol_corr r1 p1 r2 p2];
                else
                    %correlation with target and adjacent elements
                    [r1 p1] = corrcoef(npks_burst,vol1);
                    [r2 p2] = corrcoef(npks_burst,vol2);
                    
                    %save measurements and variables
                    spk_vol_corr = [spk_vol_corr; r1(2) p1(2) r2(2) p2(2) alignby pkactivity...
                        wth mean(pct_error) sum(~isnan(npks_burst)) max([varburst1 varburst2])];
                    case_name = [case_name,{ff(i).name,gapids{n}}];
                    T = maketable(ff(i).name,gapids(n),vol1,vol2,npks_burst,pkactivity);
                    dattable=[dattable;T];
                end
...
function T = maketable(name,seqid,volume1,volume2,npks_burst,pkactivity);
    [~,stid] = regexp(name,'data_');
    enid = regexp(name,'_TH');
    unitid = name(stid+1:enid-1);
    birdid = repmat({unitid(1:regexp(unitid,'_')-1)},length(npks_burst),1);
    unitid = repmat({unitid},length(npks_burst),1);
    seqid = repmat(seqid,length(npks_burst),1);
    pkactivity = repmat(pkactivity,length(npks_burst),1);
    npks_burst = (npks_burst-nanmean(npks_burst))./nanstd(npks_burst);
    T = table(npks_burst,volume1,volume2,birdid,unitid,seqid,pkactivity,'VariableNames',...
        {'spikes','volume1','volume2','birdid','unitid','seqid','activity'});