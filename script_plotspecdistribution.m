...
for i = 1:length(ff)
     ...
     for ii = 1:length(syllables)
         ...
         fvdata = [fvdata; nanmean([ydata(:).fv])];
         
         ydata = [x(:).([syllables{ii}])];
         voldata = [voldata; nanmean([ydata(:).vol])];
         
         ydata = [x(:).([syllables{ii}])];
         entdata = [entdata; nanmean([ydata(:).ent])];
         
         ydata = [x(:).([syllables{ii}])];
         pcvdata = [pcvdata; nanmean([ydata(:).pcv]-1)];
         
         ydata = [y(:).([syllables{ii}])];
         fvdata_sal = [fvdata_sal; nanmean([ydata(:).fv])];
         
         ydata = [y(:).([syllables{ii}])];
         voldata_sal = [voldata_sal; nanmean([ydata(:).vol])];
         
         ydata = [y(:).([syllables{ii}])];
         entdata_sal = [entdata_sal; nanmean([ydata(:).ent])];
         
         ydata = [y(:).([syllables{ii}])];
         pcvdata_sal = [pcvdata_sal; nanmean([ydata(:).pcv]-1)];    
     end
end
