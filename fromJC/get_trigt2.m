function trigs=get_trigt(bt,cntrng,refrac,NFFT,ADDX,TEMPFILE,CHANSPEC);
...
for ijk=1:length(files)
	...
	cntvals(:,ii)=cnt.';
        ...
        for kk=1:NCNT
            ...
            %if (DO_OR==0)
            if (kk==1)
                trig=ntrig;%keep trig if in column 1
            else
                if (cntrng(kk-1).AND==1)%not column 1, check last column for AND
                    trig = trig & ntrig;%AND: trig if both are 1
                else
                    trig = trig | ntrig;%OR: trig if either are 1 
                end
            end
        end
        ...
        if (trig)
            if(strcmp(CHANSPEC,'w'))
                ...
                tafter = 1;                        
            elseif(strcmp(CHANSPEC,'obs0'))
                tbefore = rdat.tbefore;                                   
            end
           
            if (abs(ii-lasttrig)>refracsam)
                tt=[tt;((ii*NFFT*2/fs)+tbefore)*1e3];
                %tt=[tt;((ii*NFFT*2/fs)+2)*1e3];
                lasttrig=ii;
            end
        end
    end
