...
function [] = plot_tmcourse5(crbs, ps,avls)
...
%             vls.acmean(ii)=crbs.acmean(1,cr_murun);
            ...
            vls.mudiff(ii)=vls.mumean(ii)-crbs.initmean(1);
       ...
       for ii=1:length(crbs.mulist)
           ...
           acind=crbs.aclist(ii,:);
                            ...
                            vls.acdiff(ii)=calcmn-crbs.initmean(1);
                            vls.actms(ii)=calctm;
...
if(isfield(ps,'muindvl'))
    ...
    combactms=vls.actms(ps.mulistind)-vls.zerotime;
...
if(isfield(ps,'plotlman'))
   if(ps.plotlman)
      ...
      hold on;
      plot(diffx(nzind),diffy(nzind),'Marker',ps.acmark','Color',ps.accol,'MarkerFaceColor','none','Linewidth',ps.lw)
   end
end
...
if(ps.plotsep)
    ...
    if(ps.plotinitendline)
        ...
        plot(initactms,initacvls,'Linestyle','none','Marker',ps.acmark,'Color',ps.accol,'Markersize',ps.msize,'MarkerFaceColor','none');
      plot(endactms,endacvls,'Linestyle','none','Marker',ps.acmark,'Color',ps.accol,'Markersize',ps.msize,'MarkerFaceColor','none'); 
    end
      if(ps.plotmumarker)
        plot(xvecmu,ps.muht*ones(1,length(xvecmu)),'Marker','v','Color',ps.mucol,'MarkerFaceColor','none','Linestyle','none');  
      end
      if(ps.plotmuline)
          plot(xvecmu,muvls,'Color',ps.mucol,'Linewidth',ps.lw);
          plot(xvecmu,muvecbot,'Linestyle','none','Marker',ps.mumark,'Color',ps.mucol,'Markersize',ps.msize,'MarkerFaceColor','none');
      end
      if(ps.plotrawmu)
         for ii=1:length(vls.rawmuvls)
             plot(vls.rawmutms{ii},vls.rawmuvls{ii},'o','MarkerFaceColor','none','MarkerEdgeColor',ps.mucol,'MarkerSize',ps.mumarksize);
         end
      end
    if(ps.plotallac)
        crvlcomb=[];
        ...
        for ii=1:length(nozeroind)
            ...
            hold on;  
            if(ps.plotacline)
            plot(crtm-start_time,crvl,'o','MarkerFaceColor','none','MarkerEdgeColor',ps.accol,'MarkerSize',ps.msize)
...
if(ps.plotallacpts)
            if(avls.aclist(crind,1))
                ...
                hold on;       
                 plot(crvls(:,1)-start_time,crvls(:,2)/3000,'o','MarkerFaceColor','none','MarkerEdgeColor','k','MarkerSize',1.5)
      ...
      if(ps.plotrawac)
        for ii=1:length(vls.initrwtms)
            plot(vls.initrwtmsall{ii},vls.crinitvls{ii},'o','MarkerFaceColor','none','MarkerEdgeColor','k','MarkerSize',2);
            plot(vls.endrwtmsall{ii},vls.crendvls{ii},'o','MarkerFaceColor','none','MarkerEdgeColor','k','MarkerSize',2);
        end
    end
