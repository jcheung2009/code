...
stimset(2) = generate_fxd_stimuli(wf_motif2, fs, frequency_steps, time_steps, 'motif1');
frequency_steps = [.9];
time_steps = [.6, 1.4];
stimset(3) = generate_fxd_stimuli(wf_motif2, fs, frequency_steps, time_steps, 'motif1');
frequency_steps = [1.1];
time_steps = [.6, 1.4];
stimset(4) = generate_fxd_stimuli(wf_motif2, fs, frequency_steps, time_steps, 'motif1');

% motif 2
frequency_steps = [.9, .98, 1, 1.02, 1.1];
time_steps = [1];
stimset(5) = generate_fxd_stimuli(wf_motif1, fs, frequency_steps, time_steps, 'motif2');
...
save_krank_stimuli(full_stimset, pathdata.stim_path, 'tfshift_sparse')
return
