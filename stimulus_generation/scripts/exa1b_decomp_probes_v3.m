...
syllable_idxs_temp(4:6) = sub_idxs(1,:);
% drop each syllable
syllables = data.syllables(syllable_idxs_temp);
...
syllables_temp(2).wf = song_shift(syllables_temp(2).wf,1,fshift);
...
stimset.stims(stimcount).onset = 0;
stimset.stims(stimcount).offset = length(stimset.stims(stimcount).signal);
stimset.stims(stimcount).name = ['exa1_song_a_1_sylb_fshift_fixed_' num2str(fshift)];
stimset.numstims = stimcount;
end
...
for fshift = f_shifts
syllables_temp = syllables;
syllables_temp(3).wf = song_shift(syllables_temp(3).wf,1,fshift);
motif = generate_motif_with_even_gaps(syllables_temp, data.fs, 50e-3, 250e-3);
stimcount = stimcount + 1;
stimset.stims(stimcount).signal = motif;
stimset.stims(stimcount).type = 'cutoff';
...
stimset.stims(stimcount).onset = 0;
stimset.stims(stimcount).offset = length(stimset.stims(stimcount).signal);
stimset.stims(stimcount).name = ['exa1_song_a_1_sylc_fshift_fixed_' num2str(fshift)];
stimset.numstims = stimcount;
end
...
for fshift = f_shifts
syllables_temp = syllables;
syllables_temp(4).wf = song_shift(syllables_temp(4).wf,1,fshift);
motif = generate_motif_with_even_gaps(syllables_temp, data.fs, 50e-3, 250e-3);
stimcount = stimcount + 1;
stimset.stims(stimcount).signal = motif;
stimset.stims(stimcount).type = 'cutoff';
...
stimset.stims(stimcount).onset = 0;
stimset.stims(stimcount).offset = length(stimset.stims(stimcount).signal);
stimset.stims(stimcount).name = ['exa1_song_a_1_sylx_fshift_fixed_' num2str(fshift)];
stimset.numstims = stimcount;
end
...
for fshift = f_shifts
syllables_temp = syllables;
syllables_temp(5).wf = song_shift(syllables_temp(5).wf,1,fshift);
motif = generate_motif_with_even_gaps(syllables_temp, data.fs, 50e-3, 250e-3);
stimcount = stimcount + 1;
stimset.stims(stimcount).signal = motif;
stimset.stims(stimcount).type = 'cutoff';
...
stimset.stims(stimcount).onset = 0;
stimset.stims(stimcount).offset = length(stimset.stims(stimcount).signal);
stimset.stims(stimcount).name = ['exa1_song_a_1_syly_fshift_fixed' num2str(fshift)];
stimset.numstims = stimcount;
end
...
%% save
if savetest
    save_boc_stimuli(stimset, pathdata.stim_path, ['exa1_decomposition_probes_v4'], 'db', 75)
else
