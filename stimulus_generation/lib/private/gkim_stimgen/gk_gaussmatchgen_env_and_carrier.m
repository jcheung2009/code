...
save_gaussian_components = 0  %GK

%stimbasedir = input('Enter base directory for output stimulus files: ','s')
stimbasedir = input(['Enter base directory for output stimulus files [', pwd, ']: '],'s') %GK
...
if do_ensemble_carrier_and_envelope_GN
  ...
  norm_info{nstims}.type = 'centercut_peakrms'  % GK
  % We match the rms to the ensemble rms
%  norm_info{nstims}.type = 'set_gain'   % GK %%%%
...
while ifile <= nfiles    
  ...
  % GK: This is log lv envelope %%%%%%%%%%%%%%%%%
  logenv = ensemble.envelope.loglv;    % GK
  env_len = length(logenv)             % GK
  ...
  for istim = 1:nstims      
    ...
    if norm_info{istim}.nclip/stim_length > clip_frac
      bad_clipping = 1
%      break                       % GK
      error('Too much clipping!')  % GK
      norm_info{istim}.nclip       % Gk
    end
    ...
    %tempstim = gk_make_ramp(tempstim, ramp_info);    %GK: use gk_make_ramp
...
%save(randstatefile,'st','st_env')  % GK
...
savefile = fullfile(stimbasedir,[filenameprefix, '_AM', '.mat'])  %GK
save(savefile)
