...
% -- In v2, added multikrank data file type (GK)
...
if strcmp(filetype,'k')
  chan = input('Enter channel number for krank file (start at 0) ');
  if isempty(chan); chan = 0; end
  [song,rec] = gk_readkrankf(songfile,chan);
  Fs = rec.ai_freq;
  if max(song) < 1000
    song = song*10000;
  end
...
if ~(strcmp(filetype,'w') || strcmp(filetype,'filt') || strcmp(filetype,'k'))
  Fs = input('What is sample rate (samples/sec)? [32000] ');
  if isempty(Fs); Fs = 32000; end
end
