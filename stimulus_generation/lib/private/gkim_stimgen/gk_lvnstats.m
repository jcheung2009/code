...
%smwin_dur = [0.5 1.0 2.0 4.0 10.0]  %GK
...
for ifile=1:batch_info.nfiles
  ...
  for wlen = smwin_len
    ...
    %GK CHECK
%     t_lvenv = (0:length(lvenv)-1)/Fs;
%     t_song = (0:songlen(ifile)-1)/Fs;
%     figure; plot(t_song,song,'color',[.7 .7 .7])
%     hold on
%     plot(t_lvenv,2.5*sqrt(lvenv))
%     pause
...
figure
plot(smwin_dur,DKL_gausslvnsym,'o')   %GK symbol
...
figure
plot(smwin_dur,DKL_lvnsymgauss,'o')
xlabel('LV window duration (msec)')
...
figure
plot(smwin_dur,DKL_gausslvn2,'o')
xlabel('LV window duration (msec)')
ylabel('D_{KL}(N||P) (bits)')
...
figure
plot(smwin_dur,DKL_lvngauss2,'o')
xlabel('LV window duration (msec)')
ylabel('D_{KL}(P||N) (bits)')
...
figure
plot(smwin_dur,DKL_gaussloglv,'o')
xlabel('LV window duration (msec)')
ylabel('D_{KL}(N||P) (bits)')
...
figure
plot(smwin_dur,DKL_loglvgauss,'o')
xlabel('LV window duration (msec)')
ylabel('D_{KL}(P||N) (bits)')
...
figure
plot(smwin_dur,DKL_gaussloglv2,'o')
xlabel('LV window duration (msec)')
ylabel('D_{KL}(N||P) (bits)')
...
figure
plot(smwin_dur,DKL_loglvgauss2,'o')
xlabel('LV window duration (msec)')
ylabel('D_{KL}(P||N) (bits)')
...
% GK: Overlay song and lvenv
figure
plot( (0:length(filtsong)-1)/Fs, filtsong,'color',[.7 .7 .7]) 
hold on
if ~strcmp(lvwin_type,'boxcar')
  plot( (0+offset:length(filtsong)-1+offset-wlen)/Fs, sqrt(lvenv)*2.5)
  title(['LV envelope, hanning, T=',num2str(smwin_dur),'msec'])
else
  plot( (0+wlen:length(filtsong)-1)/Fs, sqrt(lvenv)*2)
  title(['LV envelope, boxcar, T=',num2str(smwin_dur),'msec'])
end
xlabel('time(sec)')
...
savefile = input(['Enter filename for saving workspace (path will be', ...
      ' data base dir): '],'s')
...
if strncmpi(yesno,'y',1)  
  ...
  ensemble.nfiles = batch_info.nfiles;               %GK
  ...
  for iw = 1:nwin
    ...
    ensemble.envelope.loglv = loglv;      % GK %%%%%%%%%%%%%%%%%
    ensemble.envelope.lvnsong = lvnsong;  % GK %%%%%%%%%%%%%%%%%
