MAX_SAMPLES = 200000000%12500000
MAX_DA = 32767
MIN_DA = -32768
...
filt_info_dflt.F_high = 12000 %20000  
...
while ifile <= nfiles    
  ...
  for istim=1:nstims
    filename{istim} = [filenameprefix, '_', file_info{istim}.key, '_', num2str(ifile+9),'.raw']
  ...
  for istim = 1:nstims
    ...
    tempstim = gk_make_ramp(tempstim, ramp_info);
  ...
  if do_plots | do_osc_plots | do_stim_spect
    ...
    %close(hh(handle_idx))  % commented by GK
    clear td
  end
