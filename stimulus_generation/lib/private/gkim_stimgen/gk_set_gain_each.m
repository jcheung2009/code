...
% Use this if gain is different for each file. Asks for gain matrix that
% has gain values in dB for each file(song).
...
atten = input('Enter the attenuation matrix(in dB) name:  ')
while (atten > 80)
...
for ifile=1:batch_info.nfiles
  ...
  outsongname = [outsongprefix, '_G', num2str(gain(ifile)), o_ext, o_ver];
  outsongfile = fullfile(stimbasedir,outsongname);
  norm_info.setgainval = gain(ifile);
