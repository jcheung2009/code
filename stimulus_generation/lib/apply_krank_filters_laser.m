function wfout = apply_krank_filters_laser(wf, fs, varargin)
...
if set_amp
    % set amplitude
    wfout = wfout/max(wfout);
    wfout = wfout*2^15;
end
%     wfout = set_rms_for_krank(wfout, db);
% end

wfout = [zeros(round(zero_pad_length*fs_out),1); wfout(:); zeros(round(zero_pad_length*fs_out),1)];
% add noise
