function jc_autoLabelrep(batchfile,template,cntrng,syl,refrac,reprefrac,ampThresh,min_dur,min_int,EVMODE,NFFT,sm_win)
%7_23_2014: modified to only label first syllable of repeat
...
while (1)
	...
	cntvals(:,ii)=cnt.';
     ...
     %restrict trigcnts to first repeats
        ...
        for mm = 1:length(trigcnt)
            ...
            tt = (trigrep1*NFFT*2/fs)+PREDATATIME;
        end
	...
	for ii = 1:length(rdat.ttimes)
		pp=find((onsets<=rdat.ttimes(ii)*1e3)&(offsets>=rdat.ttimes(ii)*1e3));
		if (length(pp)>0)
