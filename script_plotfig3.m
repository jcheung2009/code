...
%     end
    ...
    cmd1 = ['load(''analysis/data_structures/motif_abcccb_',ff(i).name,''')'];
    cmd3 = ['load(''analysis/data_structures/motif_abcccb_',ff(i+1).name,''')'];
%     cmd4 = ['load(''analysis/data_structures/motif_abcccb_',ff(i+3).name,''')'];
    eval(cmd1);
    eval(cmd3);
%     eval(cmd4);
    
    %cmd1 = ['tb_sal = jc_tb([motif_abcccb_',ff(i).name,'(:).datenm]'',7,0);'];
    cmd2 = ['tb_cond = jc_tb([motif_abcccb_',ff(i+1).name,'(:).datenm]'',7,0);'];
%     cmd3 = ['tb_sal2 = jc_tb([motif_abcccb_',ff(i+3).name,'(:).datenm]'',7,0);'];
    %eval(cmd1);
    eval(cmd2);
%     eval(cmd3);
...
%     ind2 = find(tb_sal2 >= tb_cond(1) & tb_sal2 <= tb_cond(end));
    ...
    cmd2 = ['mBootstrapCI([motif_abcccb_',ff(i).name,'(:).firstpeakdistance])'];
    [hi lo mn] = eval(cmd2);
    ...
    cmd2 = ['mBootstrapCI([motif_abcccb_',ff(i+1).name,'(ind).firstpeakdistance])'];
    [hi lo mn] = eval(cmd2);
    plot(gca,[daycnt+spc daycnt+spc],[hi lo],mcolor,'linewidth',2);hold on;
    plot(gca,daycnt+spc,mn,mrk,'MarkerSize',8);hold on;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     cmd2 = ['mBootstrapCI([motif_abcccb_',ff(i+3).name,'(:).firstpeakdistance])'];
%     [hi lo mn] = eval(cmd2);
