...
%correlation
%seqlen = length of syllable sequence, target syll is middle
...
for i = 1:length(ff)
    ...
    for n = 1:length(gapids)
        ...
        for m = 1:size(seqons,1)
            ...
            spktms{m} = x - seqons(m,seqlen/2+1); %spike times aligned by sylloff1 
        end
        
        %align spike times by landmarks (each onset/offset in sequence)
        seqons_a = seqons-seqons(:,seqlen/2+1);%align time of each onset by gap onset
        seqoffs_a = seqoffs-seqons(:,seqlen/2+1);%align time of each offset by gap onset
        ...
        for pt = 2:size(landmarks,2)
                ...
                bound = bounds(pt-1);
                    ...
                    smooth_spiketrains = zeros(length(dur_id),seqst2+seqend2+1);
