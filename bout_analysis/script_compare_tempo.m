%summary plots for tempo bout analysis

ff = load_batchf('batch');

%% extract pitch information from all birds
lname = {};
tempo_pattern_pre = {};%each cell is one bird: average pitch at each bout position
tempo_pattern_post = {};
firsttempo_vs_lngth_corr_post = [];%corr coeffs for each bird for tempo beginning vs bout length
lasttempo_vs_lngth_corr_post = [];%corr coeffs for each bird for tempo end vs bout length
for i = 1:length(ff)
    cd(ff(i).name);
    ...
    for ii = 1:length(params.boutstructs)
        ...
        if params.boutinfo{ii} == 1 %ignore boutstructs with only spec info
            continue
        end
        ...
        firsttempo_vs_lngth_corr_post = [firsttempo_vs_lngth_corr_post; r];
        ...
        lasttempo_vs_lngth_corr_post = [lasttempo_vs_lngth_corr_post; r];
        ...
        tempo_pattern_pre = [tempo_pattern_pre; cellfun(@(x) mean(x),pre_boutctrl)];
        tempo_pattern_post = [tempo_pattern_post; cellfun(@(x) mean(x),post_boutctrl)];
...
tempo_pattern_pre_norm = cellfun(@(x) x./x(1),tempo_pattern_pre,'unif',0);%normalize average pitch at each bout position by average pitch at first position
tempo_pattern_post_norm = cellfun(@(x) x./x(1),tempo_pattern_post,'unif',0);
pre_boutchange = cellfun(@(x) sum(diff(x)),tempo_pattern_pre_norm);
post_boutchange = cellfun(@(x) sum(diff(x)),tempo_pattern_post_norm);
pre_absfv = cellfun(@(x) x(1), tempo_pattern_pre);
post_absfv = cellfun(@(x) x(1),tempo_pattern_post);
...
pre_boutchange = cellfun(@(x) sum(diff(x)),tempo_pattern_pre);
post_boutchange = cellfun(@(x) sum(diff(x)),tempo_pattern_post);
pre_absfv = cellfun(@mean, tempo_pattern_pre);
post_absfv = cellfun(@mean,tempo_pattern_post);
...
figure;hold on;
plot(1,firsttempo_vs_lngth_corr_post,'marker','o','markersize',8,'linewidth',2);hold on;
plot(2,lasttempo_vs_lngth_corr_post,'marker','o','markersize',8,'linewidth',2);hold on;
legend(lname);
y = get(gca,'ylim');
[p h] = signrank(firsttempo_vs_lngth_corr_post);
text(1,y(2),{['p=',num2str(p)]});
[p h] = signrank(lasttempo_vs_lngth_corr_post);
...
minmotif = min(cellfun(@(x) length(x),tempo_pattern_pre));
tempo_pattern_pre2 = cell2mat(cellfun(@(x) x(1:minmotif),tempo_pattern_pre,'unif',0));
tempo_pattern_pre2 = tempo_pattern_pre2./tempo_pattern_pre2(:,1);
figure;hold on;
plot(1:minmotif,tempo_pattern_pre2,'marker','o','markersize',8,'linewidth',2);hold on;
legend(lname);
bar([1:minmotif],mean(tempo_pattern_pre2),'facecolor','none','edgecolor','k','linewidth',1.5);hold on;
ylim([0.95 1]);
y = get(gca,'ylim');
for i = 1:minmotif-1
    [p h] = signrank(tempo_pattern_pre2(:,i),tempo_pattern_pre2(:,i+1));
    text(i+1,y(2),{['p=',num2str(p)]});
end
...
minmotif = min(cellfun(@(x) length(x),tempo_pattern_post));
tempo_pattern_post2 = cell2mat(cellfun(@(x) x(1:minmotif),tempo_pattern_post,'unif',0));
tempo_pattern_post2 = tempo_pattern_post2./tempo_pattern_post2(:,1);
figure;hold on;
plot(1:minmotif,tempo_pattern_post2,'marker','o','markersize',8,'linewidth',2);hold on;
legend(lname);
bar([1:minmotif],mean(tempo_pattern_post2),'facecolor','none','edgecolor','k','linewidth',1.5);hold on;
ylim([0.95 1]);
y = get(gca,'ylim');
for i = 1:minmotif-1
    [p h] = signrank(tempo_pattern_post2(:,i),tempo_pattern_post2(:,i+1));
    text(i+1,y(2),{['p=',num2str(p)]});
end
ylabel('normalized pitch');
xlabel('bout position');
...
tempo_pattern_pre = cell2mat(cellfun(@(x) x(1:minmotif),tempo_pattern_pre,'unif',0));
tempo_pattern_post = cell2mat(cellfun(@(x) x(1:minmotif),tempo_pattern_post,'unif',0));
