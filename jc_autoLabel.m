function jc_autoLabel(batchfile,template,cntrng,syl,refrac,ampThresh,min_dur,min_int,EVMODE,NFFT,sm_win,chcklbl)
...
%chcklbl: = 1, runs jc_chcklbl/jc_fndlbl 
...
%     cntrng(1).AND=0;
%     cntrng(1).BTMIN=0;
%will NOT overwrite already labelled syllables
...
while (1)
	...
	for ii = 1:length(rdat.ttimes)
		...
		if (length(pp)>0)
            if labels(pp(end)) ~= '-'%prevent labelling already labelled syllable
                continue
            else
...
if chcklbl == 1
    ...
    jc_chcklbl(batchfile,syl,result{1},result{2},result{3},result{4},result{5},CHANSPEC);
end
