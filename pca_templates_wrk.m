function [out meanVect seVect] = pca_templates_dist(in_struct,nameStr)
%
...
meanVect = zeros(length(in_struct)-1,1);
seVect = zeros(length(in_struct)-1,1);
for i=1:length(in_struct)-2;
    [neighbors pcDistance] = kNearestNeighbors(out{i+2}',out{i+1}',1);
