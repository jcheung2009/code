function  RA_burstposterior(batchfile,seqlen,minsampsize,activitythresh,...
    motorwin,mode,ifr,gap_or_syll)
%determine posterior probability of spikes within detected bursts before
%target song element; plot those cases where activity is greater than threshold 
%and more than 80% of spikes in burst have greater than 95% posterior prob
    ...
    for n = 1:length(gapids)
        ...
        if strcmp(mode,'burst')
            ...
            for ixx = 1:length(pkid)
                ...
                burstspkconfidence = cdfburstspkposterior(find(b==0.95));

                if pkactivity >= activitythresh & burstspkconfidence <= 0.2
...
function [PSTH_mn tb smooth_spiketrains spktms spkpost] = smoothtrain(spiketimes,spikeposterior,seqst,seqend,anchor,win,ifr);
