...
plot_tempospec_timecourse = input('plot tempo and spec timecourse?:','s');
if plot_tempospec_timecourse =='y'
    
    fignum = input('figure for tempo and spectral measurements over time:');
    plot_spec = input('plot spec?:','s');
    figure(fignum);hold on;
    runlength = [tb [fv_rep(:).runlength]'];
    if plot_spec == 'y'
        subtightplot(4,2,1,0.07);
    else
        subtightplot(4,1,1,0.07);
    end
...
plot_tempo_by_repeat = input('plot tempo measurements by repeat position:','s');
if plot_tempo_by_repeat == 'y'
    ...
    for i = 4:maxlength
        ...
        for ii = 1:length(fv_rep)
            if fv_rep(ii).runlength >= i
                [c lags] = xcorr(fv_rep(ii).sm(1:ceil(fv_rep(ii).off(i)*fs/1000)),'coeff');
                c = c(ceil(length(c)/2):end);
...
plot_spec_by_repeat = input('plot spectral features by repeat position:','s');
if plot_spec_by_repeat == 'y'
...
plot_repeat_patterns = input('plot repeat patterns?:','s');
if plot_repeat_patterns == 'y'
...
plot_tempospec_corr = input('plot correlation between tempo and spec:','s');
if plot_tempospec_corr == 'y'
