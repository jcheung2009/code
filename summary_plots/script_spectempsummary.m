...
for i = 1:length(params.trial)
    if isfield(params,'findnote')  
        for n = 1:length(params.findnote)
            ...
            fv_base = eval([params.findnote(n).fvstruct,params.trial(i).name]);
            fv_cond = eval([params.findnote(n).fvstruct,params.trial(i).baseline]);
    ...
    if isfield(params,'findmotif')
        for n = 1:length(params.findmotif)
            ...
            motif_base = eval([params.findmotif(n).motifstruct,params.trial(i).name]);
            motif_cond = eval([params.findmotif(n).motifstruct,params.trial(i).baseline]);
    ...
    if isfield(params,'findrepeat')
        for n = 1:length(params.findrepeat)
            ...
            rep_base = eval([params.findrepeat(n).repstruct,params.trial(i).name]);
            rep_cond = eval([params.findrepeat(n).repstruct,params.trial(i).baseline]);
    ...
    if isfield(params,'findbout')
        for n = 1:length(params.findbout)
            ...
            bout_base = eval([params.findbout(n).boutstruct,params.trial(i).name]);
            bout_cond = eval([params.findbout(n).boutstruct,params.trial(i).baseline]);
