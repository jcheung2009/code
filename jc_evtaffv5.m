function [vals trigtime hitcount] = jc_evtaffv5(batch,NT,cntrng,refrac,reprefrac,NFFT,fvalbnd,tmpdetect,USEX,evtaf4)
%7_23_2014: modified to restrict trigs to first repeats 
%reprefrac = seconds since last detected syllable in previous repeat set 
%
%7_16_2014
...
if tmpdetect == 1 %do for all triggers in tmp file (escapes + hits) 
    ...
    for i = 1:length(files)
        ...
        %restrict trigcnts to first repeats
        repref = reprefrac*fs/(2*NFFT); %convert reprefrac from seconds to tmp bins 
        tbefore = rdat.tbefore*fs;
        for mm = 1:length(trigcnt)
            repgap = 0.15*fs/(2*NFFT); %set 150 ms as max duration between repeats, change into number of tmp bins 
            firstrepind = find(diff([-repgap;trigcnt])>repgap); %find points in trig cnt where trigs are separated by more than 150 ms
            trigrep1 = trigcnt(firstrepind(1));
            if length(firstrepind) > 1
                for nn = 2:length(firstrepind)
                    if trigcnt(firstrepind(nn)) - trigcnt(firstrepind(nn)-1) > repref %only keep first repeats if they're separated by more the repeat refractory period
                        trigrep1 = [trigrep1;trigcnt(firstrepind(nn))];
                    end
                end
            end
            tt = (trigrep1*NFFT*2)+tbefore; %trig times for first repeat syllables in samples
        end
