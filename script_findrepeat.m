...
for i = 1:length(ff)
    ...
    cmd = ['fv_repA_',ff(i).name,'=','jc_findrepeat2(''batch.keep.rand'',''a'','''',fvalbnd_syllA,[0.025 0.035],1,''obs0'',''n'',''y'')'];
      cmd2 = ['fv_repB_',ff(i).name,'=','jc_findrepeat2(''batch.keep.rand'',''b'','''',fvalbnd_syllB,[0.02 0.03],1,''obs0'',''n'',''y'')'];
      %cmd3 = ['fv_repD_',ff(i).name,'=','jc_findrepeat2(''batch.keep.rand'',''d'','''',fvalbnd_syllD,[0.02 0.04],1,''w'',''n'',''y'')'];
    eval(cmd);
      eval(cmd2);
%      eval(cmd3);
