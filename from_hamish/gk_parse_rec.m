function rec = gk_parse_rec(filename);
...
% [fpath,fname,fext,ver] = fileparts(filename);
[fpath,fname,fext] = fileparts(filename);
rec.recfilename = [filename, '.rec'];
%fullfile(fpath,[fname,'.rec',ver]);

fid=fopen(rec.recfilename);
if fid == -1
...
%  disp(tline)
  ...
  if strmatch('date:',tline)
    rec.timestamp = strread(tline,'date: %s','delimiter','');
