 function [data,t_amplifier,Fs,filedata,song]=read_Intan_RHD2000_song(filename,chan)
 
 

%  vargout{1}=
 
% function [data,t_amplifier,Fs,filedata,auxchan,t_auxin]=read_Intan_RHD2000_file(filename,chan)

% read_Intan_RHD2000_file
%
% Version 1.1, 31 May 2013
%
...
amplifier_data = zeros(1, num_amplifier_samples); %whm
data=zeros(1,num_amplifier_samples); %whm;
...
%            temp = fread(fid,60,'uint16',(num_amplifier_channels-1)*60);
%    keyboard
            temp=fread(fid,[60, num_amplifier_channels],'uint16');         
            data(1,amplifier_index:(amplifier_index+59))=temp(:,chan);
%            data(1,amplifier_index:(amplifier_index+59))=temp;
%           data(:,amplifier_index:(amplifier_index+59)) = fread(fid,[60, chan],'uint16')
% keyboard
    end
...
data = 0.195 * (data - 32768); % units = microvolts
...
%         amplifier_data(i,:) = ...
            notch_filter(data, sample_rate, notch_filter_frequency, 10);
...
%     move_to_base_workspace(t_dig_in);
% end
% if (num_temp_sensor_channels > 0)
