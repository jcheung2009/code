...
switch action
  ...
  case 'filetype'
    ...
    if exist('myaction','var') & strcmp(myaction,'init')    
      % set defaults
      SALoad.filetype = 'w';
      SALoad.filetype_strs = {'w', 'obs0r', 'obs1r', 'd', 'b', 'filt', 'foo','okrank0','evtaf','Intan','MatLab(WHM)'};
      SALoad.filetype_tips = {'WAV file', 'Observer (last channel)', ...
	    'Observer (next to last channel)', 'dcp file', ...
	    'binary (mac) format', 'filt file', 'foosong file','Observer Krank file (first channel)','evtaf','Intan','MatLab(WHM)'}
