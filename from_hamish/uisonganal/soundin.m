...
elseif strcmp(filetype,'evtaf')
    [rawsong, Fs] = ReadEvTAFFile(soundfile_full,0);
elseif  strcmp(filetype(1:3),'obs')
   if length(filetype)==3
      chan_spec='0r';  %default is to return the last channel of observer fileui
   ...
   elseif strcmpi(filetype,'Intan')
    [rawsong, Fs]= IntanRHDReadSong([pathname '/'],soundfile);%% forward slash for linux    
  elseif strcmp(filetype(1:6),'okrank')
    chan_spec = filetype(end);
    [rawsong, Fs] = read_okrank_data(soundfile_full, chan_spec);

elseif strcmpi(filetype,'MatLab(WHM)');    
    [rawsong]= load(soundfile_full,'SongBout');
    rawsong=rawsong(1).SongBout;
    [Fs]=load(soundfile_full,'Fs');
    Fs=Fs.Fs(1);
