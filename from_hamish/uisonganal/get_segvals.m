...
prompt= {'Threshold','Minimum Interval (ms)', 'Minimum Duration (ms)', 'Smoothing Window'};
 
Values=inputdlg(prompt,'Resegmenting Options',1,{num2str(threshold) num2str(min_int) num2str(min_dur) num2str(sm_win)});

if threshold ~= str2num(Values{1}) 
    change_flag(thresh_i)=1;end;
if min_int ~= str2num(Values{2}); 
    change_flag(int_i)=1;end;
if min_dur ~= str2num(Values{3});         
    change_flag(dur_i)=1;end;
if sm_win ~= str2num(Values{4});
    change_flag(win_i)=1;end;

threshold=str2num(Values{1});
min_int=str2num(Values{2});
min_dur=str2num(Values{3});
sm_win=str2num(Values{4});
