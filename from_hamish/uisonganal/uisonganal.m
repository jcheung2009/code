function varargout = uisonganal(varargin)
...
global h_main h_main_spect h_main_amp h_main_labels h_labels
global win_percent
...
%control flow
global get_next get_prev SA_close interact batch_disp save_notes save_spect ...
...
h_main=figure('Tag','SAMain','WindowButtonMotionFcn', ...
...
SA.param_defaults.filter_type = 'butter';
...
SA.param_defaults.min_dur=10;

%values for calculating spectrogram
SA.param_defaults.spect_win_dur=16;
SA.param_defaults.spect_overlap = .40;  %percentage of overlap of specgram window
...
SA.param_defaults.spect_cmap_name = 'hot';
...
if strcmpi(platf,'mac')
  SA.param_defaults.do_patch = 0; % Do we show a semi-transparent
                                  % selection patch when selecting a
                                  % range or zooming?
else
  SA.param_defaults.do_patch = 1; % Do we show a semi-transparent
                                  % selection patch when selecting a
                                  % range or zooming?
end  
...
if use_new_gui_features
  ...
  [SA.loaddir, fname,fext] = fileparts(SA.loadfile);
  SA.loadfilebase = [fname fext];
  ...
  SA.single = 0;
  SA.loadfilebase = 0;
  while SA.loadfilebase == 0 | isempty(SA.batchname)
    disp('select batchfile');
    [SA.loadfilebase, SA.loaddir]=uigetfile('*','select batchfile')
    SA.loadfile = fullfile(SA.loaddir,SA.loadfilebase)
  end
  ...
  disp(' okrank0 = observer krank file (first channel)');
  disp(' evtaf = evtaf file');
  disp(' Intan');
  disp('MatLab(WHM)');
  ...
  while strcmp(SA.filetype,'null')
    ...
    elseif strcmpi(temp,'o1r')
      SA.filetype = 'obs1r';
    elseif strcmpi(temp,'okrank0')
      SA.filetype = 'okrank0';
    elseif strcmpi(temp,'evtaf');
      SA.filetype = 'evtaf';
    elseif strcmpi(temp,'Intan')
      SA.filetype= 'intan';
    elseif (strcmpi(temp,'MatLab'))
        SA.filetype='mat';
    else
...
while 1
  ...
  %get soundfile name
  if i_song > SA.n_songs & interact==0
    soundfile = '';
  ...
  % batch file
  [path_batchsongfile,soundfilename,ext] = fileparts(soundfile);   
  % This is the sound file name with no path spec.
  soundfile = [soundfilename,ext];
...
%       keyboard;
  ...
  if batch_disp == 1 | batch_print == 1
    ...
    if batch_disp
      ...
      % Turn on main window
      set(h_main,'Visible','on', 'Name', ['uisonganal: ', SA.loadfilebase])
      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    end
    ...
    SA.handles.mainLabels = h_main_labels;
    % Tag for identifying main axes 
    ...
    SA.handles.mainSpect = h_main_spect;
    % Tag for identifying main axes 
...
%    movegui(gcf,'onscreen')
  ...
  while get_next==0 & get_prev==0 & SA_close == 0 & interact == 1
    pause(.5)
    if SA_close == 1
      break;
    end
    ...
    % Got command to go to the previous song.
    elseif get_prev==1
      i_song = i_song - 1;
      if i_song < 1
        i_song = 1
