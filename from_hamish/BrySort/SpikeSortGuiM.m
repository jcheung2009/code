...
% Last Modified by GUIDE v2.5 24-Jun-2013 12:27:49
...
handles.colorCodes = {'r','b','g','c','m','k','k','k','k','k','k','k','r','b','g','c','m','k','k','k','k','k','k','k','r','b','g','c','m','k','k','k','k','k','k','k','r','b','g','c','m','k','k','k','k','k','k','k'};
handles.colorDotCodes = {'.r','.b','.g','.c','.m','.k','.k','.k','.k','.k','.k','.k','.r','.b','.g','.c','.m','.k','.k','.k','.k','.k','.k','.k','r','b','g','c','m','k','k','k','k','k','k','k','r','b','g','c','m','k','k','k','k','k','k','k'};
...
%     handles.listSingle = handles.listSingle(handles.uidx ~= idxSecond); %remove idx classification
    set(handles.togglePlotSingle,'value',0); %turn of single plotting
...
%linear discriminant 
% %uses LDA function from http://matlabdatamining.blogspot.com/2010/12/linear-discriminant-analysis-lda.html
% W = LDA(handles.icacmp,handles.idx);
% L = [ones(size(handles.icacmp,1),1),handles.icacmp]*W';
% which_column = find(handles.uidx == handles.uidx(firstIndex),1); %which column will be the same order as for unique
% % P = exp(L)./repmat(sum(exp(L),2),[1 size(W,2)]);
% in_class_distances = L(handles.idx == handles.uidx(firstIndex),which_column);
% all_class_distances = L(:,which_column);
% bins = linspace(min(in_class_distances)-abs(min(in_class_distances))/2,max(in_class_distances)+abs(max(in_class_distances))/2,99);
% disp(bins)

%mahalanobis plot (could replace linear discriminant)
in_class = handles.icacmp(handles.idx == handles.uidx(firstIndex),:);
if (size(in_class,1) > size(in_class,2))
    all_class = handles.icacmp(handles.idx ~= handles.uidx(firstIndex),:);
    all_class_distances = mahal(all_class, in_class);
    in_class_distances  = mahal(in_class, in_class);
    % % %use fastmcd to create better estimate the covariance matrix
    % options.lts = 1;
    % res = fastmcd(in_class,options); %slow enough to save for "run complex methods"
    % mean_in = mean(in_class,1);
    % all_class_distances = mahaldist(all_class,mean_in,inv(res.cov));
    % in_class_distances  = mahaldist(in_class,mean_in,inv(res.cov));
    bins = linspace(0, max(in_class_distances)*2, 99);

    all_class_hist = histc(all_class_distances,bins);
    in_class_hist = histc(in_class_distances,bins);
    cla(handles.axesLDA);
    hold(handles.axesLDA,'on');
    bar(handles.axesLDA,all_class_hist,'k','edgecolor','none');
    bar(handles.axesLDA,in_class_hist,'b','edgecolor','none');
    %plot 85% line
    x_co = find(cumsum(in_class_hist)/sum(in_class_hist) > 0.85,1,'first');
    plot(handles.axesLDA,[x_co,x_co],[0,max(in_class_hist)*1.5],'b');
    if secondIndex ~= 0
    %       second_class_distances = L(handles.idx == handles.uidx(secondIndex),which_column);
        second_class = handles.icacmp(handles.idx == handles.uidx(secondIndex),:);
        second_class_distances = mahal(second_class, in_class);
    %     second_class_distances = mahaldist(second_class,mean_in,inv(res.cov));
        second_class_hist = histc(second_class_distances,bins);
        bar(handles.axesLDA,second_class_hist,'r','edgecolor','none');
    end
    ylim(handles.axesLDA,[0,max(in_class_hist)*2]);
end

%update ISI plot
event_times = handles.times(handles.idx == handles.uidx(firstIndex),:);
bins = 0:0.001:0.03;
isi_hist = histc(diff(event_times),bins)/length(event_times);
if isempty(isi_hist)
    cla(handles.axesISI)
else
    bar(handles.axesISI,bins,isi_hist,'b');
    xlim(handles.axesISI,[0,0.01]);
    ylim(handles.axesISI,[0,0.05]);
end
...
string_set = {};
string_set{end+1} = sprintf('%d events',sum(handles.idx == handles.uidx(firstIndex)));
try
    output = gaussian_isolation(handles.icacmp,handles.idx,handles.uidx(firstIndex));
    string_set{end+1} = sprintf('P(spike|cluster) = %f',mean(output.p_event));
    string_set{end+1} = sprintf('Entropy = %f',output.p_entropy);
    string_set{end+1} = sprintf('P(false positive) = %f',output.p_false_p);
    string_set{end+1} = sprintf('P(false negative) = %f',output.p_false_n);
catch error
    string_set{end+1} = 'Could not compute metrics due to low number of events in one cluster';
end
set(handles.textStats,'String',string_set);
...
%     p = sum(log(p));
% end
% set(handles.textStats,'String',['logP(Secondary|Main) = ',num2str(p)]);
...
% --- Executes on button press in button_run_more_metrics.
function button_run_more_metrics_Callback(hObject, eventdata, handles)
% hObject    handle to button_run_more_metrics (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

outputs = sliding_gaussian_isolation(handles.icacmp,handles.idx,handles.uidx(get(handles.menuMainSpike, 'Value')));
string_set = {};
string_set{end+1} = sprintf('P(spike|cluster) = %f',mean(outputs.p_event));
string_set{end+1} = sprintf('Entropy = %f',mean(outputs.p_entropy));
string_set{end+1} = sprintf('P(false positive) = %f',mean(outputs.p_false_p));
string_set{end+1} = sprintf('P(false negative) = %f',mean(outputs.p_false_n));
msgbox(string_set);
