...
if ~strcmp(output.path{1},'lMANOnly')
    sz=length(output.HVc.signal);
end;
    
    
if ~strcmp(output.path{1},'HVcOnly')
    sz=length(output.lMAN.signal);
end;
...
%     R(length(R)+1)=output.R;
    ...
    if strcmp(output.path{1},'HVcOnly')
        [HVcAmp(i,1) HVcPPR(i,1) HVcbase(i,1)]=PPRs(output.HVc.signal{i},pulsewidth(1),debug);       
        
    elseif strcmp(output.path{1},'lMANOnly')
        [lMANAmp(i,1) lMANPPR(i,1) lMANbase(i,1)]=PPRs(output.lMAN.signal{i},pulsewidth(2),debug);
