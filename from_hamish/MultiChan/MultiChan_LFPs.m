...
%     
    ...
    for i=1:size(ChanList,1); %length(ChanList,1);
            ...
            for j=1:numtrials; %length(ChanList)
                

                    v{j}=data(startidx(j):endidx(j));    
...
%                     [S,F,T,P] = spectrogram(Result(1).song,Fs);
        %             T=T*1000*ScaleFactor(i); %ms
                    ...
                    Result(i).lfpF{j}=F;
                      ...
                      AvgLFP(1:mtxsize(1),1:mtxsize(2))=AvgLFP(1:mtxsize(1),1:mtxsize(2))+P;                    
           ...
           set(gca,'xtick',[],'ytick',[]);
%                    
           hand=subplot(SubPlotNum,5,[1:5]+((i)*5));  %ChanList subplots. 
