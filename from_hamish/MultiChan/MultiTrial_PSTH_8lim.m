    ...
    SubPlotNum=14; %Result(1).numtrials+5; 
...
%     
    filtsong = bandpass(Result(15).song,Fs,300,10000);
    [S,F,T,P] = spectrogram(filtsong,256,250,256,Fs);
...
%     keyboard;
    ...
    %Set Windows. 
    gaussFilter = gausswin(1); %gausswin(Result(1).GaussianWindow);
...
%                 keyboard;
                CC(j,k)=max(CCORR); %CCORR(round(0.5*length(CCORR)));
             end;
         end;
