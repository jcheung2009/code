 function [amp_data,t_amp,Fs,filedata,song]=jc_IntanRHDImportAll(batch) 
...
%                 end
    ...
    filedata.(fieldname).num_amplifier_channels=num_amplifier_channels;
    filedata.(fieldname).num_aux_input_channels=num_aux_input_channels;
    filedata.(fieldname).num_supply_voltage_channels=num_supply_voltage_channels;
    filedata.(fieldname).num_board_adc_channels=num_board_adc_channels;
    filedata.(fieldname).num_board_dig_in_channels=num_board_dig_in_channels;


     amp_data.(fieldname) = amplifier_data;
     t_amp.(fieldname) = t_amplifier;
     Fs.(fieldname)=sample_rate;
     song.(fieldname)=board_adc_data; %(1,:);
