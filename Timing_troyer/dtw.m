...
%% set defaults
params.metric = @mnsqrdev2;%use jc_dot?
...
%max normalizes each time frequency bin for both template and exemplar
%before computing distance matrix
dtwtemplate = bsxfun(@rdivide,abs(template),max(abs(template)));
dtwexemplar = bsxfun(@rdivide,abs(exemplar),max(abs(exemplar)));
for j=1:size(dtwtemplate,2)
    imin = max(1,j-mlimneg);
    imax = min(size(dtwexemplar,2),j+mlimpos);
    M(imin:imax,j) = feval(params.metric,dtwtemplate(:,j),dtwexemplar(:,imin:imax));
end    
